...
public class TestOLVerificationLibraryPS {
	...
	public void testParseSubjectSuccess() throws Exception {
		...
		expectedPreds.add(new Predicate("uri:Age",Operation.GREATERTHANOREQUAL,new Attribute(18)));
		expectedPreds.add(new Predicate("uri:DateOfBirth",Operation.LESSTHANOREQUAL,new Attribute(Util.fromRFC3339UTC("2003-03-23T00:00:00"))));
	...
	public void testVerifierBadConstructor() throws Exception {
		...
		fail("Should throw SetupException");
	}

	@Test
	public void testVerifierSetupExceptions() throws Exception {
		MSpublicParam mockPublicParam = MockFactory.mockPublicParam();
		...
		try {
			...
			fail("Should throw SetupException, conflictingAttr");
		...
		try {
			...
			fail("Should throw SetupException, wrongPublicParam");
		} catch (SetupException e) {
		}
		try {
			credentialVerifierModule.setup(new PabcPublicParameters(new HashSet<>(Arrays.asList(new AttributeDefinitionBoolean("test","test"))), wrongPublicParam.getEncoded()), null, seed);
			fail("Should throw SetupException");
		} catch (SetupException e) {
		}
	}
	...
	public void testOlPSLibraryMethodsNoSetup()  {
		...
		try{
			verifier.verifyOlPsDerivedProof("testToken", new HashMap<>(),"nonce","expDate",123);
			fail();
		}catch (IllegalStateException e){
		}
		try{
			verifier.verifyOlPsDerivedProofRange("testToken", new HashMap<>(),new HashMap<>(), predicates,"nonce","expDate",123);
			fail();
		}catch (IllegalStateException e){
		}
	}
	...
	public void testWrongAttributes() throws Exception {
		...
		assertThat(library.verifyOlPsDerivedProof(TEST_TOKEN,attr,"nonce","expDate",123),is(OLVerificationLibraryResult.INVALID_ATTRIBUTES));
	}
	...
	public void testWrongAttributesRange() throws Exception {
		...
		assertThat(library.verifyOlPsDerivedProofRange(TEST_RANGE_TOKEN,wrongAttr,goodTokenMap,new HashSet<>(),"nonce","expDate",123),is(OLVerificationLibraryResult.INVALID_ATTRIBUTES));
		assertThat(library.verifyOlPsDerivedProofRange(TEST_RANGE_TOKEN,goodAttr,wrongTokenMap,new HashSet<>(),"nonce","expDate",123),is(OLVerificationLibraryResult.INVALID_ATTRIBUTES));
		assertThat(library.verifyOlPsDerivedProofRange(TEST_RANGE_TOKEN,goodAttr,goodTokenMap,new HashSet<>(),"nonce","expDate",123),is(OLVerificationLibraryResult.INVALID_SIGNATURE));
	}
	...
	public void testSetupBadInput() throws Exception {
		...
		PabcIdP idp = mock(PabcIdP.class);
		doReturn(new PabcPublicParameters(new HashSet<>(),"Wrong")).when(idp).getPabcPublicParam();
		idps.add(idp);
		OLVerificationLibraryPS verifier = new OLVerificationLibraryPS();
	...
	public void testSetupWrongSchemeName() throws Exception {
		...
		List<PabcIdP> idps = new ArrayList<PabcIdP>();
		PabcIdP idp = mock(PabcIdP.class);
		doReturn(new PabcPublicParameters(new HashSet<>(),new PSpublicParam(1,new PSauxArg("WrongName",new HashSet<>())).getEncoded())).when(idp).getPabcPublicParam();
		idps.add(idp);
		OLVerificationLibraryPS verifier = new OLVerificationLibraryPS();
		verifier.setup(idps,seed);
		fail("Should throw SetupException");
	...
	public void testSetupConflictingAttributeNames() throws Exception {
		...
		List<PabcIdP> idps = new ArrayList<PabcIdP>();
		PabcIdP idp = mock(PabcIdP.class);
		doReturn(new PabcPublicParameters(new HashSet<>(),param)).when(idp).getPabcPublicParam();
		idps.add(idp);
		OLVerificationLibraryPS verifier = new OLVerificationLibraryPS();
		verifier.setup(idps,seed);
		fail("Should throw SetupException");
