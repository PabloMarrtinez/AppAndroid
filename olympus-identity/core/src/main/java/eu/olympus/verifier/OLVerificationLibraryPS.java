...
public class OLVerificationLibraryPS implements OLVerificationLibrary {
    ...
    public void setup(PabcPublicParameters publicParameters, MSverfKey olympusVerificationKey, byte[] seed) throws SetupException {
        attributeDefinitions=publicParameters.getAttributeDefinitions();
        try {
            schemePublicParameters = new PSpublicParam(publicParameters.getEncodedSchemePublicParam());
        } catch (InvalidProtocolBufferException e) {
    ...
    @Override
    public OLVerificationLibraryResult verifyOlPsDerivedProof(String token, Map<String, Attribute> revealedAttributes, String nonce, String presExpDate, long epoch) {
        if(multiSignatureScheme==null) {
        ...
        if(!multiSignatureScheme.verifyZKtoken(reconstructedToken,olympusVerificationKey,nonce+presExpDate,revealedAttributesMessage))
            return OLVerificationLibraryResult.INVALID_SIGNATURE;
        return OLVerificationLibraryResult.VALID;
    }

    @Override
    public OLVerificationLibraryResult verifyOlPsDerivedProofRange(String token, Map<String, Attribute> revealedAttributes, Map<String, RangePredicateToken> rangeTokens, Set<Predicate> rangePredicates, String nonce, String presExpDate, long epoch) {
        if(multiSignatureScheme==null) {
            throw new IllegalStateException("It is necessary to run setup before using this method");
        }
        MSzkToken reconstructedToken;
        ...
        Map<String, ZpElement> revealedZpAttributes=transformAttributes(revealedAttributes);
        MSmessage revealedAttributesMessage=new PSmessage(revealedZpAttributes,builder.getZpElementFromEpoch(epoch));
        if(!multiSignatureScheme.verifyZKtokenModified(reconstructedToken,olympusVerificationKey,nonce+presExpDate, revealedAttributesMessage,Vp))
            return OLVerificationLibraryResult.INVALID_SIGNATURE;
