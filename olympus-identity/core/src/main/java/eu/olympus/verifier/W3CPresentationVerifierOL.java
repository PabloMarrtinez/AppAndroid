...
public class W3CPresentationVerifierOL implements W3CPresentationVerifier {
    ...
    public W3CVerificationResult verifyPresentationToken(String token, Policy policy) {
        ...
        switch ((String) type){
            case "OlPsDerivedProof":
                ...
                return verifyOlPsDerivedProof(proof,revealedAttributes,(String) nonce,Util.toRFC3339UTC(presExpirationDate), (Long) epoch);
            case "OlPsDerivedProofRange":
                if (rangePredicates.isEmpty())
                    return W3CVerificationResult.INVALID_SIGNATURE;
                return verifyOlPsDerivedProofRange(proof,revealedAttributes,rangePredicates, (String) nonce,Util.toRFC3339UTC(presExpirationDate), (Long) epoch);
            default:
                return W3CVerificationResult.INVALID_TOKEN;
        }
    }

    private W3CVerificationResult verifyOlPsDerivedProof(Proof proof, Map<String, Attribute> revealedAttributes, String nonce, String presExpDate, long epoch) {
        Object pv=proof.getProofValue();
        if(!(pv instanceof String))
            return W3CVerificationResult.INVALID_TOKEN;
        if (verificationLibrary.verifyOlPsDerivedProof((String) pv, revealedAttributes, nonce,presExpDate, epoch) == OLVerificationLibraryResult.VALID) {
    ...
    private W3CVerificationResult verifyOlPsDerivedProofRange(Proof proof, Map<String, Attribute> revealedAttributes, Set<Predicate> rangePredicates, String nonce, String presExpDate, long epoch) {
        Object pv=proof.getProofValue();
        if(!(pv instanceof String))
            return W3CVerificationResult.INVALID_TOKEN;
        ...
        if(rangeTokens==null)
            return W3CVerificationResult.INVALID_SIGNATURE;
        if (verificationLibrary.verifyOlPsDerivedProofRange((String) pv, revealedAttributes, rangeTokens, rangePredicates, nonce,presExpDate, epoch) == OLVerificationLibraryResult.VALID) {
            return W3CVerificationResult.VALID;
        } //case INVALID_ATTRIBUTES: return W3CVerificationResult.INVALID_ATTRIBUTES; Not really possible because of previous check in "parseCredentialSubject"
        return W3CVerificationResult.INVALID_SIGNATURE;
    }
