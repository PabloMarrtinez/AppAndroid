...
public abstract class AbstractPestoIdP implements PestoBasedIdP {
    ...
    public byte[] finishRegistration(String username, byte[] cookie, PublicKey publicKey, byte[] signature, long salt, String idProof) throws UserCreationFailedException {
        ...
        } catch (AuthenticationFailedException | UserCreationFailedException e){
            logger.info("Failed to finish registration",e);
    ...
    public boolean deleteAttributes(String username, byte[] cookie, long salt, byte[] signature, List<String> attributes) throws AuthenticationFailedException {
        ...
        try {
            ...
            throw new AuthenticationFailedException("Failed to parse attributes", e);
        }
    }
